...
export async function initCommand(): Promise<void> {
  ...
  if (schemaPath) {
    ...
    const shouldSync = await p.confirm({
      message: "Sync CCM Feedback models to your Prisma schema?",
    });
    ...
    if (shouldSync) {
      try {
        ...
        p.outro("Fix the errors above and re-run `ccm-feedback init`.");
        process.exit(1);
      }
    }
  } else {
    p.log.warn("No schema.prisma file found. You will need to add the models manually.");
    p.log.info("See the documentation: https://github.com/ccmdesign/ccm-feedback-tool#prisma-schema-1");
  }
